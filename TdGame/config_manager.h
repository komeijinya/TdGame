...
public:
	...
	bool load_game_config(const std::string& path)
	{
		...
		parse_tower_template(axman_template, cJSON_GetObjectItem(json_tower, "axeman"));
...
private:
	...
	void parse_tower_template(TowerTemplate& tpl, cJSON* json_root)
	{
		...
		cJSON* json_upgrafe_cost = cJSON_GetObjectItem(json_root, "upgrafe_cost");
		...
		parse_number_array(tpl.upgrade_cost, 9, json_upgrafe_cost);
	...
	void parse_enemy_template(EnemyTemplate& tpl, cJSON* json_root)
	{
		...
		if (json_hp && json_hp->type == cJSON_Object)
			tpl.hp = json_hp->valuedouble;
		if (json_speed && json_speed->type == cJSON_Object)
			tpl.speed = json_speed->valuedouble;
		if (json_damage && json_damage->type == cJSON_Object)
			tpl.damage = json_damage->valuedouble;
		if (json_reward_ratio && json_reward_ratio->type == cJSON_Object)
			tpl.reward_ratio = json_reward_ratio->valuedouble;
		if (json_recover_interval && json_recover_interval->type == cJSON_Object)
			tpl.recover_intensity = json_recover_interval->valuedouble;
		if (json_recover_range && json_recover_range->type == cJSON_Object)
			tpl.recover_range = json_recover_range->valuedouble;
		if (json_recover_intensity && json_recover_intensity->type == cJSON_Object)
